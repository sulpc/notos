...
#define util_swap(type, a, b)                do { type _t_##a##_##b = a; a = b; b = _t_##a##_##b; } while (0)
...
#define util_containerof(type, field, ptr)   ((type*)((uint8_t*)(ptr) - ((const uint8_t*)&((type*)0)->field - (const uint8_t*)0)))
 ...
 * @return uint32_t
 */
inline uint32_t util_bytes2uint_le(uint8_t* buf, uint8_t size)
{
    union {
        uint32_t u32;
        uint8_t  u8[4];
...
inline uint32_t util_bytes2uint_be(uint8_t* buf, uint8_t size)
{
    union {
        uint32_t u32;
        uint8_t  u8[4];
...
static inline void util_dumpbytes(const uint8_t* data, uint32_t len)
{
    if (len == 0 || data == NULL) {
        return;
    }
