...
// init queue node
#define util_queue_init_static(node) {.prev = node, .next = node}
...
#define util_queue_foreach_safe(node_, queue_)                                                                         \
    for (util_queue_node_t* node_ = (queue_)->next, *_next_ = node_->next; node_ != (queue_);                          \
         node_ = _next_, _next_ = _next_->next)
